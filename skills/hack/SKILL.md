...
## ⛔ CRITICAL: SCOPE GUARD — MUST LOAD FIRST

**BEFORE loading any other skill or starting any hunt, you MUST load scope-guard.**

```bash
# THIS IS THE FIRST COMMAND OF EVERY SESSION:
skill scope-guard
```

scope-guard checks:
1. Does SCOPE_ACTIVE.md exist?
2. Is the target in scope?
3. Are test accounts configured?
4. Are there any OOS exclusions?

If any check fails → **STOP**. Do NOT proceed to recon, scanning, or exploitation.
...
- 不要把这里的技巧用于未授权攻击。
- **SCOPE GUARD is mandatory.** If you haven't loaded `scope-guard` first, you are NOT authorized to hunt.
