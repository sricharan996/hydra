...
fetch_undetected_urls() {
  ...
  case $api_key_index in
    1) api_key="${VIRUSTOTAL_API_KEY_1:-${VIRUSTOTAL_API_KEY:-}}" ;;
    2) api_key="${VIRUSTOTAL_API_KEY_2:-${VIRUSTOTAL_API_KEY:-}}" ;;
    *) api_key="${VIRUSTOTAL_API_KEY_3:-${VIRUSTOTAL_API_KEY:-}}" ;;
  esac
  if [ -z "$api_key" ]; then
    echo -e "\033[1;31mError: export VIRUSTOTAL_API_KEY (free key: virustotal.com)\033[0m"
    return
  fi
